...
- [ ] **AUTH-01**: Player can log in with phone number + password via `POST /auth/player/login`
- [ ] **AUTH-02**: Login calls Frappe whitelisted API (`verify_player_password`) — single HTTP call, no Frappe session
- [ ] **AUTH-03**: Login returns enriched response (tokens + display_name, avatar, gender, XP)
- [ ] **AUTH-04**: Admin can log in with email + password via `POST /auth/admin/login` (existing Frappe User flow)
- [ ] **AUTH-05**: Token refresh works for both player and admin tokens via `POST /auth/refresh`
...
- [ ] **REG-03**: OTP stored in Redis with 5-minute TTL, static "1111" stub with pluggable provider interface
- [ ] **REG-04**: Phone reservation during OTP window prevents duplicate registration attempts
- [ ] **REG-05**: OTP resend with 60-second cooldown via `POST /auth/player/register/resend`
- [ ] **REG-06**: Player Profile, wallet, and initial Redis state created on successful registration
...
- [ ] **MIGR-01**: JWT `sub` = Player Profile docname (PLAYER-00001), `mobile` claim for phone number
