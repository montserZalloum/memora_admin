...
must_haves:
  truths:
    - "Migration documentation explains how to move FastAPI+Redis to separate server"
    - "Documentation covers network configuration changes"
    - "Documentation includes rollback procedure"
  artifacts:
    - path: "docs/server-migration.md"
      provides: "Server migration guide"
      contains: "migration"
  key_links: []
---

<objective>
Create server migration documentation for relocating Redis and FastAPI to a separate server.

Purpose: Enable future scalability by documenting the steps to move the FastAPI sidecar and Redis to dedicated infrastructure when traffic demands it.

Output:
- Comprehensive migration guide with prerequisites, steps, and verification
- Network configuration changes (firewall, nginx)
...
<task type="auto">
  <name>Task 1: Create server migration documentation</name>
  <files>
    docs/server-migration.md
  </files>
  <action>
...
```
  ...
  <verify>
    - Documentation file exists at `docs/server-migration.md`
    - Contains current and target architecture diagrams
    - Includes step-by-step migration procedure
    - Has rollback section
    - Includes troubleshooting guide
  </verify>
  <done>
    - Complete server migration documentation
...
<verification>
1. `docs/server-migration.md` exists with complete content
2. Documentation covers all aspects: preparation, migration, verification, rollback
3. Network configuration changes are documented
4. Troubleshooting section addresses common issues
</verification>

<success_criteria>
- Server migration documentation exists with steps to relocate Redis/FastAPI to separate server
- Documentation covers network configuration changes (firewall, nginx upstream)
- Rollback procedure is clearly documented
- Verification checklist provided for post-migration testing
</success_criteria>
