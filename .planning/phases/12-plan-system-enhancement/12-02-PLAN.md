---
phase: 12-plan-system-enhancement
plan: 02
type: execute
wave: 2
depends_on: ["12-01"]
files_modified:
  - memora_admin/memora_admin/services/build/plan_generator.py
autonomous: true

must_haves:
  truths:
    - "Plan JSON manifest contains subjects with total_lessons, total_tracks, is_free_preview"
    - "Subject hierarchy JSON has Plan Overrides applied (hidden content removed, is_free adjusted)"
    - "Unit content JSON contains topics with nested lessons metadata"
    - "Lesson files are shared at root level (not duplicated per plan)"
    - "is_free_preview correctly derived from visible free units/topics"
  artifacts:
    - path: "memora_admin/memora_admin/services/build/plan_generator.py"
      provides: "Plan JSON generation with Plan Overrides"
      exports: ["generate_plan_json"]
      min_lines: 200
  key_links:
    - from: "plan_generator.py"
      to: "Memora Plan Overrider"
      via: "frappe.get_all with plan filter"
      pattern: "Memora Plan Overrider"
    - from: "plan_generator.py"
      to: "storage backend"
      via: "file dict output for publisher"
      pattern: "filename.*content"
---

<objective>
Create Plan-centric JSON generator with Plan Overrides and is_free_preview derivation

Purpose: Generate plan-centric JSON files for mobile app consumption. Each plan gets its own folder with manifest, subject hierarchies (with Plan Overrides applied), and unit content files. Lesson files are shared across plans.

Output:
- `plan_generator.py` with `generate_plan_json(plan_id)` function
- Plan manifest JSON structure
- Subject hierarchy JSON with Plan Overrides applied
- Unit content JSON with topic/lesson metadata
</objective>

<execution_context>
@/root/.claude/get-shit-done/workflows/execute-plan.md
@/root/.claude/get-shit-done/templates/summary.md
</execution_context>

<context>
@.planning/PROJECT.md
@.planning/milestones/v1.2-ROADMAP.md

# Existing patterns
@memora_admin/memora_admin/services/build/generator.py
@memora_admin/memora_admin/doctype/memora_plan_overrider/memora_plan_overrider.json
@memora_admin/memora_admin/doctype/memora_academic_plan/memora_academic_plan.json
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create plan_generator.py with manifest generation</name>
  <files>
    memora_admin/memora_admin/services/build/plan_generator.py
  </files>
  <action>
Create `plan_generator.py` with the Plan JSON generation logic.

```python
"""
Plan-centric JSON Generator for mobile app consumption.

Generates hierarchical JSON files organized by plan:
- plans/{plan_id}/manifest.json: Plan metadata + subject list
- plans/{plan_id}/subjects/{subject_id}/_h.json: Subject hierarchy (with Plan Overrides)
- plans/{plan_id}/subjects/{subject_id}/units/{unit_id}_c.json: Unit content
- lessons/{lesson_id}.json: Shared lesson content (not per-plan)
"""

from __future__ import annotations

import json
import logging
from datetime import datetime, timezone
from typing import Any

import frappe

logger = logging.getLogger(__name__)

SCHEMA_VERSION = 1


def generate_plan_json(plan_id: str) -> list[dict]:
    """
    Generate all JSON files for an academic plan.

    Args:
        plan_id: The Memora Academic Plan document name (e.g., "PLAN-00001")

    Returns:
        List of file dictionaries with:
        - filename: str (path including plan folder)
        - content: str (JSON string)
    """
    files: list[dict] = []

    try:
        plan_doc = frappe.get_doc("Memora Academic Plan", plan_id)
    except frappe.DoesNotExistError:
        logger.warning(f"Plan {plan_id} not found, skipping")
        return files

    # Load Plan Overrides once for efficiency
    overrides = _load_plan_overrides(plan_id)

    # Get plan subjects from child table
    plan_subjects = [ps.subject for ps in plan_doc.plan_subjects or []]

    if not plan_subjects:
        logger.warning(f"Plan {plan_id} has no subjects, skipping")
        return files

    # Generate manifest
    manifest_data = _generate_manifest(plan_doc, plan_subjects, overrides)
    files.append({
        "filename": f"plans/{plan_id}/manifest.json",
        "content": _to_json(manifest_data),
    })

    # Generate per-subject files
    for subject_id in plan_subjects:
        subject_files = _generate_subject_files(plan_id, subject_id, overrides)
        files.extend(subject_files)

    return files


def _load_plan_overrides(plan_id: str) -> dict[str, dict]:
    """
    Load all Plan Overrides for a plan.

    Returns dict keyed by (ref_doctype, ref_name) with action.
    """
    overrides_raw = frappe.get_all(
        "Memora Plan Overrider",
        filters={"plan": plan_id},
        fields=["ref_doctype", "ref_name", "action"]
    )

    overrides = {}
    for ovr in overrides_raw:
        key = (ovr["ref_doctype"], ovr["ref_name"])
        overrides[key] = {"action": ovr["action"]}

    return overrides


def _is_hidden(overrides: dict, doctype: str, name: str) -> bool:
    """Check if item is hidden by Plan Override."""
    key = (doctype, name)
    return overrides.get(key, {}).get("action") == "Hide"


def _is_override_free(overrides: dict, doctype: str, name: str) -> bool | None:
    """Check if item has 'Set Free' override. Returns None if no override."""
    key = (doctype, name)
    if overrides.get(key, {}).get("action") == "Set Free":
        return True
    return None


def _generate_manifest(plan_doc: Any, subject_ids: list[str], overrides: dict) -> dict:
    """Generate manifest.json for the plan."""
    version = int(datetime.now(timezone.utc).timestamp())

    # Get grade and major titles
    grade_title = ""
    major_title = ""

    if plan_doc.grade:
        grade_title = frappe.get_cached_value("Memora Grade", plan_doc.grade, "grade_title") or ""
    if plan_doc.major:
        major_title = frappe.get_cached_value("Memora Major", plan_doc.major, "major_title") or ""

    # Build subjects array
    subjects = []
    for subject_id in subject_ids:
        if _is_hidden(overrides, "Memora Subject", subject_id):
            continue

        try:
            subject_doc = frappe.get_doc("Memora Subject", subject_id)
        except frappe.DoesNotExistError:
            continue

        # Calculate stats and is_free_preview
        stats = _calculate_subject_stats(subject_id, overrides)

        subjects.append({
            "id": subject_id,
            "title": subject_doc.subject_title,
            "alias_title": getattr(subject_doc, "alias_title", None),
            "image": _relative_path(subject_doc.image),
            "total_lessons": stats["total_lessons"],
            "total_tracks": stats["total_tracks"],
            "is_premium": bool(getattr(subject_doc, "is_premium", False)),
            "is_free_preview": stats["is_free_preview"],
            "hierarchy_url": f"/files/cdn/plans/{plan_doc.name}/subjects/{subject_id}/_h.json"
        })

    return {
        "schema_version": SCHEMA_VERSION,
        "version": version,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "plan_id": plan_doc.name,
        "title": plan_doc.plan_name,
        "grade_id": plan_doc.grade,
        "grade_title": grade_title,
        "major_id": plan_doc.major,
        "major_title": major_title,
        "season_id": plan_doc.season,
        "subjects": subjects,
    }


def _calculate_subject_stats(subject_id: str, overrides: dict) -> dict:
    """
    Calculate subject statistics with Plan Overrides applied.

    Returns:
        dict with total_lessons, total_tracks, is_free_preview
    """
    total_lessons = 0
    total_tracks = 0
    is_free_preview = False

    # Get tracks
    tracks = frappe.get_all(
        "Memora Track",
        filters={"subject": subject_id, "is_published": 1},
        fields=["name"]
    )

    for track in tracks:
        if _is_hidden(overrides, "Memora Track", track["name"]):
            continue

        total_tracks += 1

        # Get units
        units = frappe.get_all(
            "Memora Unit",
            filters={"track": track["name"], "is_published": 1},
            fields=["name", "is_free"]
        )

        for unit in units:
            if _is_hidden(overrides, "Memora Unit", unit["name"]):
                continue

            # Check unit is_free (with override)
            unit_is_free = _is_override_free(overrides, "Memora Unit", unit["name"])
            if unit_is_free is None:
                unit_is_free = bool(unit.get("is_free"))

            if unit_is_free:
                is_free_preview = True

            # Get topics
            topics = frappe.get_all(
                "Memora Topic",
                filters={"unit": unit["name"], "is_published": 1},
                fields=["name", "is_free"]
            )

            for topic in topics:
                if _is_hidden(overrides, "Memora Topic", topic["name"]):
                    continue

                # Check topic is_free (with override)
                topic_is_free = _is_override_free(overrides, "Memora Topic", topic["name"])
                if topic_is_free is None:
                    topic_is_free = bool(topic.get("is_free"))

                if topic_is_free:
                    is_free_preview = True

                # Count lessons in topic
                lesson_count = frappe.db.count(
                    "Memora Lesson",
                    filters={"topic": topic["name"]}
                )
                total_lessons += lesson_count

    return {
        "total_lessons": total_lessons,
        "total_tracks": total_tracks,
        "is_free_preview": is_free_preview,
    }


def _generate_subject_files(plan_id: str, subject_id: str, overrides: dict) -> list[dict]:
    """Generate hierarchy and unit content files for a subject."""
    files: list[dict] = []

    try:
        subject_doc = frappe.get_doc("Memora Subject", subject_id)
    except frappe.DoesNotExistError:
        return files

    # Generate _h.json (hierarchy with Plan Overrides applied)
    hierarchy_data = _generate_hierarchy(subject_doc, overrides)
    files.append({
        "filename": f"plans/{plan_id}/subjects/{subject_id}/_h.json",
        "content": _to_json(hierarchy_data),
    })

    # Generate unit content files
    tracks = frappe.get_all(
        "Memora Track",
        filters={"subject": subject_id, "is_published": 1},
        fields=["name"]
    )

    for track in tracks:
        if _is_hidden(overrides, "Memora Track", track["name"]):
            continue

        units = frappe.get_all(
            "Memora Unit",
            filters={"track": track["name"], "is_published": 1},
            fields=["name"]
        )

        for unit in units:
            if _is_hidden(overrides, "Memora Unit", unit["name"]):
                continue

            unit_content = _generate_unit_content(plan_id, subject_id, unit["name"], overrides)
            files.append({
                "filename": f"plans/{plan_id}/subjects/{subject_id}/units/{unit['name']}_c.json",
                "content": _to_json(unit_content),
            })

            # Generate lesson files (shared, check if exists first is done by publisher)
            topics = frappe.get_all(
                "Memora Topic",
                filters={"unit": unit["name"], "is_published": 1},
                fields=["name"]
            )

            for topic in topics:
                if _is_hidden(overrides, "Memora Topic", topic["name"]):
                    continue

                lessons = frappe.get_all(
                    "Memora Lesson",
                    filters={"topic": topic["name"]},
                    fields=["name"]
                )

                for lesson in lessons:
                    lesson_file = _generate_lesson_json(lesson["name"])
                    if lesson_file:
                        files.append(lesson_file)

    return files


def _generate_hierarchy(subject_doc: Any, overrides: dict) -> dict:
    """Generate _h.json subject hierarchy with Plan Overrides applied."""
    version = int(datetime.now(timezone.utc).timestamp())

    tracks_data = []
    tracks = frappe.get_all(
        "Memora Track",
        filters={"subject": subject_doc.name, "is_published": 1},
        fields=["name", "track_title", "sort_order", "is_linear", "image"],
        order_by="sort_order asc"
    )

    for track in tracks:
        if _is_hidden(overrides, "Memora Track", track["name"]):
            continue

        units_data = []
        units = frappe.get_all(
            "Memora Unit",
            filters={"track": track["name"], "is_published": 1},
            fields=["name", "unit_title", "sort_order", "is_linear", "is_free"],
            order_by="sort_order asc"
        )

        for unit in units:
            if _is_hidden(overrides, "Memora Unit", unit["name"]):
                continue

            # Apply is_free override
            unit_is_free = _is_override_free(overrides, "Memora Unit", unit["name"])
            if unit_is_free is None:
                unit_is_free = bool(unit.get("is_free"))

            units_data.append({
                "id": unit["name"],
                "title": unit["unit_title"],
                "sort_order": unit["sort_order"] or 0,
                "is_linear": bool(unit.get("is_linear")),
                "is_free": unit_is_free,
                "content_url": f"/files/cdn/plans/{subject_doc.name}/subjects/{subject_doc.name}/units/{unit['name']}_c.json"
            })

        tracks_data.append({
            "id": track["name"],
            "title": track["track_title"],
            "sort_order": track["sort_order"] or 0,
            "is_linear": bool(track.get("is_linear")),
            "image": _relative_path(track.get("image")),
            "units": units_data,
        })

    return {
        "schema_version": SCHEMA_VERSION,
        "version": version,
        "subject_id": subject_doc.name,
        "title": subject_doc.subject_title,
        "is_linear": bool(getattr(subject_doc, "is_linear", True)),
        "tracks": tracks_data,
    }


def _generate_unit_content(plan_id: str, subject_id: str, unit_id: str, overrides: dict) -> dict:
    """Generate unit content JSON with topics and lessons."""
    version = int(datetime.now(timezone.utc).timestamp())

    try:
        unit_doc = frappe.get_doc("Memora Unit", unit_id)
    except frappe.DoesNotExistError:
        return {"error": f"Unit {unit_id} not found"}

    # Apply is_free override
    unit_is_free = _is_override_free(overrides, "Memora Unit", unit_id)
    if unit_is_free is None:
        unit_is_free = bool(unit_doc.is_free)

    topics_data = []
    topics = frappe.get_all(
        "Memora Topic",
        filters={"unit": unit_id, "is_published": 1},
        fields=["name", "topic_title", "sort_order", "is_linear", "is_free"],
        order_by="sort_order asc"
    )

    for topic in topics:
        if _is_hidden(overrides, "Memora Topic", topic["name"]):
            continue

        # Apply is_free override
        topic_is_free = _is_override_free(overrides, "Memora Topic", topic["name"])
        if topic_is_free is None:
            topic_is_free = bool(topic.get("is_free"))

        lessons = frappe.get_all(
            "Memora Lesson",
            filters={"topic": topic["name"]},
            fields=["name", "lesson_title", "bit_index"],
            order_by="name asc"
        )

        lessons_data = [
            {
                "id": lesson["name"],
                "title": lesson["lesson_title"],
                "bit_index": lesson.get("bit_index") or 0,
                "content_url": f"/files/cdn/lessons/{lesson['name']}.json"
            }
            for lesson in lessons
        ]

        topics_data.append({
            "id": topic["name"],
            "title": topic["topic_title"],
            "sort_order": topic["sort_order"] or 0,
            "is_linear": bool(topic.get("is_linear")),
            "is_free": topic_is_free,
            "lessons": lessons_data,
        })

    return {
        "schema_version": SCHEMA_VERSION,
        "version": version,
        "unit_id": unit_id,
        "title": unit_doc.unit_title,
        "is_linear": bool(unit_doc.is_linear),
        "is_free": unit_is_free,
        "topics": topics_data,
    }


def _generate_lesson_json(lesson_name: str) -> dict | None:
    """Generate lesson JSON (shared across plans)."""
    try:
        lesson_doc = frappe.get_doc("Memora Lesson", lesson_name)
    except frappe.DoesNotExistError:
        logger.warning(f"Lesson {lesson_name} not found, skipping")
        return None

    stages = []
    for stage in lesson_doc.stages or []:
        stage_data = {
            "stage_id": stage.stage_id,
            "stage_type": stage.stage_type,
            "is_skippable": bool(stage.is_skippable),
            "config": _parse_stage_config(stage.config_json),
        }
        stages.append(stage_data)

    lesson_data = {
        "schema_version": SCHEMA_VERSION,
        "version": int(datetime.now(timezone.utc).timestamp()),
        "lesson_id": lesson_doc.name,
        "title": lesson_doc.lesson_title,
        "base_xp": lesson_doc.base_xp or 10,
        "max_hearts": lesson_doc.max_hearts or 3,
        "bit_index": lesson_doc.bit_index or 0,
        "is_reviewable": bool(lesson_doc.is_reviewable),
        "stages": stages,
    }

    return {
        "filename": f"lessons/{lesson_doc.name}.json",
        "content": _to_json(lesson_data),
    }


def _relative_path(url: str | None) -> str | None:
    """Convert full URL to relative path."""
    if not url:
        return None
    if url.startswith("/"):
        return url
    if "://" in url:
        path_start = url.find("://") + 3
        slash_pos = url.find("/", path_start)
        if slash_pos != -1:
            return url[slash_pos:]
    return url


def _parse_stage_config(config_json_str: str | None) -> dict:
    """Safely parse stage config JSON string."""
    if not config_json_str:
        return {}
    try:
        return json.loads(config_json_str)
    except (json.JSONDecodeError, TypeError):
        return {}


def _to_json(data: dict) -> str:
    """Convert dict to formatted JSON string with UTF-8 support."""
    return json.dumps(data, ensure_ascii=False, indent=2)
```

Key design points:
- Plan Overrides loaded once per plan for efficiency
- `is_free_preview` derived from visible free units/topics AFTER overrides applied
- Lesson files go to shared `lessons/` folder (not per-plan)
- Hierarchy files have Plan Overrides applied (hidden items removed, is_free adjusted)
  </action>
  <verify>
1. Run `python -c "import memora_admin.memora_admin.services.build.plan_generator"` to verify no import errors
2. In bench console, test:
```python
from memora_admin.memora_admin.services.build.plan_generator import generate_plan_json
files = generate_plan_json("PLAN-00001")  # Use a valid plan ID
print(f"Generated {len(files)} files")
for f in files[:3]:
    print(f"  {f['filename']}")
```
  </verify>
  <done>
`plan_generator.py` exists with `generate_plan_json()` that produces manifest, hierarchy, and unit content files with Plan Overrides applied
  </done>
</task>

<task type="auto">
  <name>Task 2: Fix hierarchy content_url paths</name>
  <files>
    memora_admin/memora_admin/services/build/plan_generator.py
  </files>
  <action>
Review and fix the content_url generation in `_generate_hierarchy()`. The current implementation has a bug where it uses `subject_doc.name` instead of `plan_id` for the path.

Find this line in `_generate_hierarchy`:
```python
"content_url": f"/files/cdn/plans/{subject_doc.name}/subjects/{subject_doc.name}/units/{unit['name']}_c.json"
```

The function needs access to `plan_id`. Update the function signature and call:

1. Change `_generate_hierarchy(subject_doc, overrides)` to `_generate_hierarchy(plan_id, subject_doc, overrides)`

2. Update the call in `_generate_subject_files`:
```python
hierarchy_data = _generate_hierarchy(plan_id, subject_doc, overrides)
```

3. Fix the content_url:
```python
"content_url": f"/files/cdn/plans/{plan_id}/subjects/{subject_doc.name}/units/{unit['name']}_c.json"
```

This ensures the path correctly points to the plan's folder structure.
  </action>
  <verify>
In bench console, verify the generated hierarchy has correct content_url paths:
```python
from memora_admin.memora_admin.services.build.plan_generator import generate_plan_json
import json
files = generate_plan_json("PLAN-00001")
for f in files:
    if "_h.json" in f["filename"]:
        data = json.loads(f["content"])
        if data.get("tracks"):
            for track in data["tracks"]:
                for unit in track.get("units", []):
                    print(unit.get("content_url"))
        break
```
  </verify>
  <done>
Hierarchy content_url paths correctly reference the plan folder structure
  </done>
</task>

</tasks>

<verification>
1. Module import succeeds without errors
2. `generate_plan_json(plan_id)` returns list of file dicts
3. Files include:
   - `plans/{plan_id}/manifest.json`
   - `plans/{plan_id}/subjects/{subject_id}/_h.json`
   - `plans/{plan_id}/subjects/{subject_id}/units/{unit_id}_c.json`
   - `lessons/{lesson_id}.json` (shared)
4. Plan Overrides are applied:
   - Hidden items excluded from output
   - `is_free` reflects overrides
5. `is_free_preview` is True if any visible unit/topic has is_free=True
</verification>

<success_criteria>
- `plan_generator.py` exists with complete implementation
- Generates correct folder structure per v1.2-ROADMAP.md spec
- Plan Overrides (Hide, Set Free) are correctly applied
- `is_free_preview` derivation works correctly
- No import or runtime errors
</success_criteria>

<output>
After completion, create `.planning/phases/12-plan-system-enhancement/12-02-SUMMARY.md`
</output>
