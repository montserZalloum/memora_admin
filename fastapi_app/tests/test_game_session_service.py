...
class TestStartSession:
	...
	async def test_tc_gs_01_start_creates_hash_with_ttl(
		...
		key = f"{test_prefix}gamesession:{TEST_USER}"
...
class TestCompleteSession:
	...
	async def test_tc_gs_06_complete_sets_progress_bit_first_completion(
		...
		progress_key = f"{test_prefix}progress:{TEST_USER}:{TEST_SUBJECT}:v{TEST_VERSION}"
		bit_value = await redis_client.getbit(progress_key, 5)
