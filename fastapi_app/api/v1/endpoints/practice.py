...
from fastapi_app.services import practice, practice_map
...
async def start_session(
	...
	if not has_subject:
		...
		if not has_all_tracks:
			# Check practice-only grant
			has_practice = await access_service.check_access(player_id, f"PRAC-SUB-{body.subject_id}")
			if not has_practice:
	...
	if free_content_scope is not None:
		allowed = practice.resolve_allowed_free_topics(
			map_data,
			body.track_ids,
			free_content_scope[0],
			free_content_scope[1],
		)
		if not allowed:
			raise HTTPException(
				status_code=status.HTTP_403_FORBIDDEN,
...
async def submit_results(
	...
	lock = redis_client.lock(
		f"memora:practice:submit:{player_id}",
		timeout=10,
	)
