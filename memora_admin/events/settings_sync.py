...
def on_settings_updated(doc, method):
	...
	try:
		payload = json.dumps({
			...
			"max_streak_multiplier_percent": doc.max_streak_multiplier_percent if doc.max_streak_multiplier_percent is not None else 50,
			"max_devices_per_player": doc.max_devices_per_player if doc.max_devices_per_player is not None else 3,
			...
			"session_timeout_days": doc.session_timeout_days if doc.session_timeout_days is not None else 30,
		})
