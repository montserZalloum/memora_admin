#!/usr/bin/env bash
# Wrapper script for the archive executor cron job.
# Sources environment variables from /etc/memora-archive.env, then runs
# the archive executor inside the dedicated venv.
#
# Usage:
#   chmod +x run_archive.sh
#   # Add to crontab:
#   0 2 * * * /home/corex/aurevia-bench/apps/memora_admin/archive_executor/run_archive.sh
#
...
cd "$APP_DIR"
exec "$VENV_PYTHON" -m archive_executor.run
